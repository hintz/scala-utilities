...
/* Adapted from https://github.com/trananh/word2vec-scala/blob/master/Word2Vec.scala
 * Reader for binary output format of the word2vec tool for word embeddings
 */
...
class Word2Vec {
  ...
  def this(filename: String, limit: Integer = Int.MaxValue, normalize: Boolean = true) {
    this; load(filename, limit, normalize)
  }
...
object TestWord2Vec extends App {
   // Load word2vec model from binary file.
    val model = new Word2Vec("../AIPHES_Data/WordEmbeddings/word2vec/denews-vectors.bin", 100000)
    println(model.vector("Haus").mkString(", "))
    println(model.vector("Tür").mkString(", "))
}